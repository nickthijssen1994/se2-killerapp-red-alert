	...
	public class TestUserContext : ITestUserInterface
	{
		...
		public TestUserContext()
		{
			Users = new List<TestUser>
			{
				new TestUser()
				{
					UserID = 1, Username = "nthijssen", Password = "123456789", IsOnline = true,
					LastOnline = DateTime.Now
				},
				new TestUser()
				{
					UserID = 2, Username = "nickthijssen1994", Password = "wachtwoord", IsOnline = true,
					LastOnline = DateTime.Now
				},
				new TestUser()
				{
					UserID = 3, Username = "nickthijssen", Password = "hallohallo", IsOnline = false,
					LastOnline = DateTime.Now.AddDays(-1)
				},
				new TestUser()
				{
					UserID = 4, Username = "nthijssen1994", Password = "password123", IsOnline = false,
		...
		public void ChangePassword(TestUser user, string newPassword)
		{
			foreach (var User in Users)
			{
				if (user == User) User.Password = newPassword;
			}
		}
		...
		public void DeleteUser(TestUser user)
		{
			foreach (var User in Users)
			{
				if (user == User) Users.Remove(User);
			}
		}
		...
		{
			var FoundUsers = new List<TestUser>();

			foreach (var User in Users)
			{
				if (User.Username.Contains(searchterm)) FoundUsers.Add(User);
			}
